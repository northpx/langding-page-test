      ...
      <div className="flex space-x-6 justify-between">
        {features.map((item, idx) => (
          <div key={idx} className="relative flex flex-col items-center text-center gap-2">
            {/* Badge */}
            {item.badge && (
              <span className="absolute md:-top-2 -right-2 bg-red-600 text-[5px] md:text-xs leading-2 uppercase text-white px-0 md:px-2 py-0.5 rounded-[3px] md:rounded-full">
                {item.badge}
              </span>
            )}
