      ...
      <div className="flex space-x-6 justify-between">
        {features.map((item, idx) => (
          <div key={idx} className="relative flex flex-col items-center text-center gap-2">
            {/* Badge */}
            {item.badge && (
              <span className="absolute -top-1 -right-1 md:-top-2 md:-right-2 bg-red-600 text-[5px] md:text-xs leading-2 uppercase text-white px-[2.67px] py-[1.33px] md:px-[2px] md:py-0.5 rounded-[3px] md:rounded-full">
                {item.badge}
              </span>
            )}
