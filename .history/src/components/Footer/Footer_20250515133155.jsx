            ...
            <div className="flex flex-col gap-3" key={item.id}>
              ...
              <ul className="flex flex-col gap-2">
                {item.content.map((content,index) => {
                  return (
                    <li key={index} className="relative text-[12px] font-normal leading-4 md:text-sm md:leading-5 text-[#D2D2D2] w-fit block after:block after:content-[''] after:absolute after:h-[1px] after:bg-white after:w-full after:scale-x-0 hover:after:scale-x-100 after:transition-all after:duration-300 after:origin-">
                      {content}
                    </li>
                  );
                })}
