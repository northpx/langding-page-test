        ...
        <div className="flex items-center justify-between px-5 py-3 md:py-5 md:px-10">
          ...
          <div className="flex gap-6 items-center flex-shrink-0">
            <div className="flex gap-2 items-center justify-center">
              ...
              <img src={logo} alt="" className="w-auto h-8 object-cover overflow-hidden" />
            </div>
            {/* {pc links} */}
            <ul className="hidden lg:flex text-[#D2D2D2]">
              {menuLink.map((item) => {
                return (
                  <li className="px-[18px] py-2 uppercase text-sm leading-5 font-medium first:text-[#FFC700] first:bg-[#383A42]" key={item.id}>
                    {item.name}
                  </li>
                );
              })}
